...
class LocalAnalysis():
    ...
    def create_parser(self):
        ...
        parser.add_argument("--numchanges", nargs=1, type=int)
        return parser
    ...
    def load(self):
        ...
        self.num_changes = self.args.numchanges[0]
        ...
        self.log('PET Prototypes are chosen from ' + str(len(set(self.prototype_img_identity))) + ' number of classes.')
        self.log('Their class identities are: ' + str(self.prototype_img_identity))

        # confirm prototype connects most strongly to its own class
        print("weight size", self.ppnet.fc2.weight.size())
        ...
        #if np.sum(self.prototype_max_connection == self.prototype_img_identity_mri) == self.ppnet.num_prototypes:
        ...
        #if np.sum(self.prototype_max_connection == self.prototype_img_identity_fdg) == self.ppnet.num_prototypes:
    ...
    def get_correct_prediction_mri(self):
        return self.predicted_cls_mri == self.correct_cls_mri

    def get_correct_prediction_mri(self):
        return self.predicted_cls_pet == self.correct_cls_pet

    def analyze(self):
        # ICXEL - preprocessing already done when creating test images
        aopc_weights = self.ppnet.fc2.weight.detach().cpu().numpy()
        ...
        print("logits size:", logits_mri,"min distances size:", min_distances_mri.size() )
        _, distances_mri = self.ppnet.ppnet1.push_forward(images_test_mri)
        _, distances_pet = self.ppnet.ppnet2.push_forward(images_test_pet)
        print("distances size:", distances_mri.size())
        ...
        idx_mri = 0
        self.predicted_cls_mri = tables_mri[idx_mri][0]
        self.correct_cls_mri = tables_mri[idx_mri][1]
        self.log('Predicted MRI: ' + str(self.predicted_cls_mri))
        self.log('Actual: ' + str(self.correct_cls_mri))
        original_img_mri = self.save_preprocessed_img(os.path.join(self.save_analysis_path, 'original_img.png'),
                                            images_test_mri, idx_mri)

        idx_pet = 0                                  
        self.predicted_cls_pet = tables_pet[idx_pet][0]
        self.correct_cls_pet = tables_pet[idx_pet][1]
        self.log('Predicted PET: ' + str(self.predicted_cls_pet))
        self.log('Actual: ' + str(self.correct_cls_pet))
        ...
        for i in range(1,9):
            ...
            self.log('last layer connection with predicted class mri: {0}'.format(self.ppnet.ppnet1.last_layer.weight[self.predicted_cls_mri][sorted_indices_act_mri[-i].item()]))
        ...
        for i in range(1,9):
            ...
            self.save_prototype_self_activation(os.path.join(self.save_analysis_path, 'most_activated_prototypes_pet',
                                                        'top-%d_activated_prototype_self_act.png' % i),
                                        self.start_epoch_number, sorted_indices_act_pet[-i].item(), 'fdg')
            if(self.num_changes == i):
                self.save_prototype_original_img_without_prototype(fname=os.path.join(self.save_new_prototypes_mri_path,
                                                                    'top-%d_activated_hidden_prototype.png' % i),
                                                epoch=self.start_epoch_number,
                                                index=sorted_indices_act_mri[-i].item(),
                                                bbox_height_start=self.prototype_info[sorted_indices_act_mri[-i].item()][1],
                                                bbox_height_end=self.prototype_info[sorted_indices_act_mri[-i].item()][2],
