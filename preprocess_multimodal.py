...
def preprocess(x, mean, std):
    #assert x.size(1) == 3
    y = dict()
    y['t1'] = torch.zeros_like(x['t1'])
    for i in range(x['t1'].size(1)):
        y['t1'][:, i, :, :] = (x['t1'][:, i, :, :] - mean[i]) / std[i]
    y['fdg'] = torch.zeros_like(x['fdg'])
    for i in range(x['fdg'].size(1)):
        y['fdg'][:, i, :, :] = (x['fdg'][:, i, :, :] - mean[i]) / std[i]
    return y
...
def undo_preprocess(x, mean, std):
    ...
    y['t1'] = torch.zeros_like(x['t1'])
    for i in range(x['t1'].size(1)):
        y['t1'][:, i, :, :] = x['t1'][:, i, :, :] * std[i] + mean[i]
    y['fdg'] = torch.zeros_like(x['fdg'])
    for i in range(x['t1'].size(1)):
        y['fdg'][:, i, :, :] = x['fdg'][:, i, :, :] * std[i] + mean[i]
    return y
