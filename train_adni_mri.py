...
def get_mri_image_transform(is_training):
    ...
    img_transforms.append(tio.CropOrPad((139, 139, 1)))
...
class AdniDateset(Dataset):
            ...
            with h5py.File(filename, mode='r') as fin: # added by Icxel
                ...
                #pet = g["PET"]["FDG"]["data"][:]
                    ...
                    for i in range(21):
                        ...
                        indx = indx[-1]//2 - 10
                        subject_to_append = subject[0,:,:,indx] # 68 is half, -+10
                        for label in target_labels:
                            target = g.attrs[label]
                            targets[label].append(target)
    ...
    def __getitem__(self, index: int):
        ...
        if self.transform is not None:
            ...
            #print("unsqueeze:", subject.unsqueeze(0).size())
            img = self.transform(subject.unsqueeze(-1))
            #img = self.transform(img)
        img = img[0,:,:,0]
        #img = img[0,:,:,69]  05.03.2022
        image_numpy = img.numpy() # added Icxel commented 05.03.2022
        rgb_img = torch.from_numpy(np.stack((image_numpy,)*3, axis=0)) # added Icxel commented 05.03.2022
        #print("img size", rgb_img.size(), img.size())
        data_point.append(rgb_img)
